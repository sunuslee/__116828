...
boot_alloc(uint32_t n)
{
    ...
    if(n == 0)
            return nextfree;
    n = ROUNDUP(n, PGSIZE);
    if(0) // i don't know how to check this yet.
    {
...
mem_init(void)
{
	...
	// Your code goes here:
    // sunus
    // Wed Nov 30 15:11:50 CST 2011
    pages = (struct Page *)boot_alloc((npages + 1) * sizeof(struct Page));
...
page_init(void)
{
        ...
        // free pages!
        size_t i;
        struct Page *prev, *p_pages;
        int n;
        page_free_list = prev = &pages[1];
        DEBUG_1(1, 0, pages);
        DEBUG_1(1, 0, npages);
        DEBUG_1(1, 0, npages_basemem);
        DEBUG_1(1, 0, (IOPHYSMEM/PGSIZE));
        DEBUG_1(1, 0, (EXTPHYSMEM/PGSIZE));
        cprintf("KER: %08x\n",(uint32_t)ROUNDUP(g_nextfree - KERNBASE, PGSIZE)/PGSIZE);
        for (i = 0; i < npages; i++) 
        {
                p_pages = &pages[i];
                if(i == 0)
                        ...
                        goto NON_FREE;
                else if(i <= (uint32_t)ROUNDUP(g_nextfree - KERNBASE, PGSIZE)/PGSIZE)
                        goto NON_FREE;
                else
                        goto FREE;
FREE:
